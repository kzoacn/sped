...
void aes_prove(const uint8_t* w, const uint8_t* u, uint8_t** V, const uint8_t* in,
  ...
  case 512:
      aes_prove_512(w, u, V, in, out, chall, a_tilde, b_tilde, params);
    break;
  case 320:
      aes_prove_320(w, u, V, in, out, chall, a_tilde, b_tilde, params);
    break;
  case 256:
...
uint8_t* aes_verify(const uint8_t* d, uint8_t** Q, const uint8_t* chall_2, const uint8_t* chall_3,
  ...
  switch (params->faest_param.lambda) {
  case 512:
      return aes_verify_512(d, Q, chall_2, chall_3, a_tilde, in, out, params);
  case 320:
      return aes_verify_320(d, Q, chall_2, chall_3, a_tilde, in, out, params);
  case 256:
